...
//        _PosOfCenter ("", float) = 0
                        ...
                        float2 texCoord : TEXCOORD) : SV_Target
    {
        ...
        for (uint part = 0; part < BODYPIX_PART_COUNT; part++)
        {
            ...
            score = smoothstep(0, 1, score);
            
            // acc += HueToRGB((float)part / BODYPIX_PART_COUNT) * score;
            // acc += (float)part / BODYPIX_PART_COUNT * score;

            if (part == BODYPIX_PART_LEFT_FACE || part == BODYPIX_PART_RIGHT_FACE) acc += score;
            else acc += 0.5 * score;
        }

        // float score = BodyPix_EvalPart(mask, BODYPIX_PART_TORSO_FRONT);
        // score = smoothstep(0.47, 0.57, score);
        // acc += HueToRGB((float)BODYPIX_PART_TORSO_FRONT / BODYPIX_PART_COUNT) * score;
        
        // score = BodyPix_EvalPart(mask, BODYPIX_PART_RIGHT_LOWER_ARM_FRONT);
        // score = smoothstep(0.47, 0.57, score);
        // acc += HueToRGB((float)BODYPIX_PART_RIGHT_LOWER_ARM_FRONT / BODYPIX_PART_COUNT) * score;
        //
        // score = BodyPix_EvalPart(mask, BODYPIX_PART_RIGHT_LOWER_ARM_BACK);
        // score = smoothstep(0.47, 0.57, score);
        // acc += HueToRGB((float)BODYPIX_PART_RIGHT_LOWER_ARM_BACK / BODYPIX_PART_COUNT) * score;
